...
player_details <- sample(player_ids, size = 10) %>%
  map(~get_player(player_id = ., competition_id)) %>%
  map(~extract_player_details(response = .))

# save
ind <- 1:length(player_details)
nr_of_players_per_group <- 10
split_ind <- split(ind, ceiling(seq_along(ind)/nr_of_players_per_group))

group_nrs <- as.character(1:length(split_ind))

split_player_details <-  split_ind %>%
  map(~player_details[.]) # if not split memory will probably be not sufficient enough

walk2(.x = split_player_details,
      .y = group_nrs,
      .f = ~save_events(list_player_details = .x, group_nr = .y))
